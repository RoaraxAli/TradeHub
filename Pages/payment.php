...
if (!isset($_GET['trade_id']) || !is_numeric($_GET['trade_id']) || !isset($_GET['method']) || $_GET['method'] !== 'company_inspection') {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $query = "UPDATE trades SET status = 'accepted', trade_method = 'company_inspection' WHERE id = ?";
            $stmt = $conn->prepare($query);
            ...
            $amount = 6.00; // $1 platform fee + $5 shipping
            $stmt->bind_param("iids", $trade_id, $_SESSION['user_id'], $amount, $payment_method);
            $stmt->execute();

            // Send message to requester
            $message = "I have accepted your trade request for {$trade['offered_item_title']} in exchange for {$trade['requested_item_title']}. The trade method is Company Inspection & Delivery, paid via " . ($payment_method === 'card' ? 'Credit/Debit Card' : 'Cash on Delivery') . ". Please prepare your item for inspection and delivery.";
            ...
            // Commit transaction
            $conn->commit();
            header("Location: trades.php?success=Trade accepted, payment processed, and message sent to requester");
            exit();
        } catch (Exception $e) {
            $conn->rollback();
            $errors[] = "Failed to process payment and accept trade: " . $e->getMessage();
        }
    }
}
...
<div class="min-h-screen bg-slate-50">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto bg-white rounded-3xl shadow-lg p-8">
            <h1 class="text-2xl font-bold text-slate-900 mb-4">Complete Trade Payment</h1>
            <p class="text-slate-600 mb-6">Please provide payment details for trading <?php echo htmlspecialchars($trade['offered_item_title']); ?> with <?php echo htmlspecialchars($trade['requested_item_title']); ?></p>
